...
stateDiagram-v2
    ...
    loading --> drivingToStore : [products >= orderQty]
...
**`productionRate` and `initialDaysBuffer`** together control the distributor stock. The inflow formula is `max(0, productionRate - products / initialDaysBuffer)`, which runs at full rate when the stock is empty and reaches zero when `products = productionRate × initialDaysBuffer`. At equilibrium, the stock stabilizes near `(productionRate - demand) × initialDaysBuffer`.
...
**Linear throttle on `productionInflow`.** A proportional throttle (`productionRate × MIN(1, target/products)`) converges to equilibrium where inflow equals demand — not where `products` equals the target. With `productionRate` (100) much larger than typical demand (≈ 15/day for 3 stores), equilibrium drifts to over 1,300 units. The linear formula `max(0, productionRate - products/initialDaysBuffer)` reaches zero inflow at the target level, bringing equilibrium close to `(productionRate - demand) × initialDaysBuffer` regardless of demand rate.
